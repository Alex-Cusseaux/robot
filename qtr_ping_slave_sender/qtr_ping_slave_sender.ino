...
//read ping
  ...
  mm = ((duration/2) / 58 )*10;     // Divide by 29.1 or multiply by 0.0343
  mm_constrained = constrain(mm, 20, 4000);
  //mm_constrained = 365;
  ...
  //  qtrrc.read(sensorValues); instead of unsigned int position = qtrrc.readLine(sensorValues);
   ...
   Serial.println(mm_constrained);
   sprintf(buf, "%04d%04d", position, mm_constrained);
   //or sprintf(buf, "04d" /n", position);
   //change buf[4] to buf [5] above
  reading =String(position);
